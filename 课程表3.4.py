...
while True:
    ...
    if schedule[today_variant][pointer][1] in SETTINGS["AFTERSCHOOL"]:
        ...
        if schedule[today_variant][pointer_next][1] in SETTINGS["CLASS"]: # next = class
            ...
            if delta_time == SETTINGS['提醒时间']:
                text = f'只有{delta_minute}分{delta_time}秒就上{schedule[today_variant][pointer_next][1]}课了，做好准备'
                win32api.MessageBox(0,text,'课程信息',win32con.MB_ICONASTERISK)
                print('hi')
            elif delta_time == SETTINGS['提醒时间']//2:
                text = f'只有{delta_minute}分{delta_time}秒就上{schedule[today_variant][pointer_next][1]}课了，做好准备!'
                win32api.MessageBox(0,text,'课程信息',win32con.MB_ICONWARNING)
            ...
            if delta_time == SETTINGS['提醒时间']:
                text = f'只有{delta_minute}分{delta_time}秒就上{schedule[today_variant][pointer_next][1]}了，做好准备'
                win32api.MessageBox(0,text,'课程信息',win32con.MB_ICONASTERISK)
